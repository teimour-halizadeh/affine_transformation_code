...
def data_analyses(F, Z):
    '''
    Analyzing the data that Cloud has obtained during solving the secure MPC. 
    We check here whether the rank condition for the data holds or not.
    '''
    l = np.size(F, 0)
    I = np.size(F, 1)
    ...
    return (rankDeltaF, rankDeltaZ, rankDeltaFDeltaZ, svd_DeltaF,
             svd_DeltaZ, svd_DeltaFDeltaZ, DeltaF, DeltaZ)
...
def theorem_6(F, Z, n_rank, m_eff):
    r'''
    Here we have Theorem 6 implementation that 

     Cloud identifies $\hat{A}  :=  TA^{N_c}T^{-1}$ with an unknown invertible matrix $T \in \R^{n\times n}$ 
     if there exists $\Theta \in \R^{(I-2) \times n}$ such that
    $$
		E_0 \Theta &= I_n \quad 
		\Delta\tilde{Z}^* \Theta &= 0 ,
    $$
		with $\Delta\tilde{Z}^*$ in \eqref{eq:data_matrices_F}.
    '''
    l = np.size(F, 0)
    I = np.size(F, 1)
